pipeline {
    ...
    stages {
        ...
        stage('Deploy to Kubernetes') {
            steps {
                withCredentials([azureServicePrincipal(AZ_K8S_KEY_ID)]) {
                    ...
                    sh 'kubectl apply -f ./kubernetes/api_deployment.yml -n nsdevsu-prod'
                    sh 'kubectl rollout restart deployment/devsuprueba-prod -n nsdevsu-prod'
                    sh 'sleep 10'
                }                   
            }
        }
        stage('Deploy to APIM') {
            steps {
                script {
                    sh 'az apim api import -g rg-devsu-dev-centralUs-001 --service-name apim-devsu-dev-centralUs-001 --subscription-key-header-name X-Parse-REST-API-Key --subscription-key-query-param-name subscription-key --api-id openapi-definition-prod --api-version prod --api-version-set-id 62ca714fb19f79aae4479f96 --path / --specification-url http://20.96.234.156/v3/api-docs.yaml --specification-format OpenApiJson'
                }                   
            }
        }
    }
